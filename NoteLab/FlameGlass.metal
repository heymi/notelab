...
[[ stitchable ]] half4 flameGlass(float2 position, half4 color, float2 size, float time, float2 tilt) {
    ...
    float surfaceLevel = 0.56 - flow.y * 0.20;
    float surfaceTilt = flow.x * (uv.x - 0.5) * 0.58;
    float fillMask = smoothstep(surfaceLevel + surfaceTilt - 0.04, surfaceLevel + surfaceTilt + 0.08, uv.y);

    float2 poolUv = uv;
    poolUv.x += sin(slow + uv.y * 4.0) * 0.030 - flow.x * 0.20;
    poolUv.y += sin(slow * 0.8 + uv.x * 5.0) * 0.018 + flow.y * 0.10;
    ...
    float2 slosh = float2(flow.x * 0.32, flow.y * -0.18);
    ...
    float field = (bottomPool * 1.18 + risingDrops * 1.22 + bridge * 0.86) * liquidWindow * fillMask;
    float liquid = smoothstep(0.26, 0.48, field);
    liquid = pow(liquid, 0.62);
    ...
    float3 rgb = base + liquidColor * liquid * 1.68 + liquidColor * surface * 0.28 + glassEdge + meniscus;
    rgb *= 0.68 + inner * 0.50;
